...
class App extends Component {
	...
	load() {
		...
		if (userInfo) {
			...
			this.socket = io(process.env.REACT_APP_NODE_SOCKET_URL ? process.env.REACT_APP_NODE_SOCKET_URL : 'http://node:7000/');
	...
	componentDidMount() {
		...
		setTimeout(
			() => document.getElementById('newMessageInput') && document.getElementById('newMessageInput').focus(),
			200
		);
	}
